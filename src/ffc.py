...
class FFCResnetBlock(nn.Module):
    def __init__(self, dim, padding_type, norm_layer, activation_layer=nn.ReLU, dilation=1,
        ...
        super().__init__()
        self.conv1 = FFC_BN_ACT(dim, dim, kernel_size=3,ratio_gin =1/4,ratio_gout =3/4, padding=dilation, dilation=dilation,
                                norm_layer=norm_layer,
                                activation_layer=activation_layer,
                                padding_type=padding_type,
                                *conv_kwargs)
        self.conv2 = FFC_BN_ACT(dim, dim, kernel_size=3, ratio_gin =1/4,ratio_gout = 3/4, padding=dilation, dilation=dilation,
                                norm_layer=norm_layer,
                                activation_layer=activation_layer,
                                padding_type=padding_type,
                                *conv_kwargs)
    ...
    def forward(self, x):
        ...
        x_g = x_g.reshape(1,64,64,192)
        x_g = self.linear1(x_g)
        out = x_l +  x_g
        out = self.linear(out)
        # out1 = out[0]
        out = out.reshape(1,256,64,64)
