...
from src.base import get_activation, BaseDiscriminator
from src.spatial_transform import LearnableSpatialTransformWrapper
from src.squeeze_excitation import SELayer
...
class FFC(nn.Module):
    ...
    def forward(self, x):
        x = x.reshape(1,70,70,256)
        x1 = self.Linear1(x)
        x2 = self.Linear2(x)
        x = x.reshape(1,256,70,70)
        x1 = x1.reshape(1,64,70,70)
        x2 = x2.reshape(1,192,70,70)
        x = (x1,x2)
...
class FFCResnetBlock(nn.Module):
    def __init__(self, dim, padding_type, norm_layer, activation_layer=nn.ReLU, dilation=1,
        ...
        super().__init__()
        self.conv1 = FFC_BN_ACT(dim, dim, kernel_size=3, ratio_gin = 1,ratio_gout=1,padding=dilation, dilation=dilation,
                                norm_layer=norm_layer,
                                activation_layer=activation_layer,
                                padding_type=padding_type,
                                **conv_kwargs)
        self.conv2 = FFC_BN_ACT(dim, dim, kernel_size=3, ratio_gin = 1,ratio_gout=1,padding=dilation, dilation=dilation,
                                norm_layer=norm_layer,
                                activation_layer=activation_layer,
                                padding_type=padding_type,
                                **conv_kwargs)
...
class FFCResNetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,
        ...
        model = [nn.ReflectionPad2d(3),
                 FFC_BN_ACT(input_nc, ngf, kernel_size=7, padding=0, norm_layer=norm_layer,ratio_gin = 1, ratio_gout = 1,
        ...
        for i in range(n_downsampling):
            ...
            cur_conv_kwargs['ratio_gout'] = 1
            model += [FFC_BN_ACT(min(max_features, ngf * mult),
                                 min(max_features, ngf * mult * 2),
                                 kernel_size=3, ratio_gin = 1,stride=2, padding=1,
                                 norm_layer=norm_layer,
                                 activation_layer=activation_layer,
                                 **cur_conv_kwargs)]
